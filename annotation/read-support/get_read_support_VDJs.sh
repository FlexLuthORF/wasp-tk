...
function run_map_ccs_to_pers {
    cat ${fofn_file} | while read sample asm_bam chr2_gene chr2_import chr22_gene chr22_import igh_gene igh_import ighc_gene ighc_import ccs_bam
    do
        ...
        cat ${mask_ref} ${pers_contigs} > ${outd}/ccs_to_pers/pers_ref.fasta
...
function get_read_support_vdj3 {
    ...
    do
        ...
        for gene_type in "chr2" "chr22" "igh"
        do
        ...
        do
            ...
            if [[ -f "$import_out" ]]; then
                ...
                for i in "${!header_cols[@]}"; do
                    case "${header_cols[$i]}" in
                        "contig") contig_col=$i ;;
                        C-EXON_[1-9]_start) exon_cols["${header_cols[$i]}"]=$i ;;
                        C-EXON_[1-9]_end) exon_cols["${header_cols[$i]}"]=$i ;;
                    esac
                done
...
# Uncomment to use get_read_support_ighc
