...
def process_locus(sample_id, bam_path, locus, loci, ref, bed_dir, allele_ref_dir):
    ...
    make_command = f"python make_gene_file.py --sample {sample_id} {locus} {loci} \"+-\" {bed_dir} {ref} {make_gene_file_output} -b {bam_path}"
    ...
    import_command = f"python import_from_assemblies.py {locus} {loci} \"+-\" {make_gene_file_output} {ref} {bed_dir} {allele_ref_dir} {import_from_assembly_output}"
    subprocess.run(import_command, shell=True, check=True)
    return import_from_assembly_output

def process_sample(sample_id, bam_path, ref, bed_dir, allele_ref_dir):
    loci_options = [
        ('IGH', 'igh'),
        ('IGH', 'ighc'),
        ('IGK', 'chr2'),
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(process_locus, sample_id, bam_path, locus, loci, ref, bed_dir, allele_ref_dir) for locus, loci in loci_options]
        results = [future.result() for future in concurrent.futures.as_completed(futures)]
    

def main(sample_id, bam_path, ref, bed_dir, allele_ref_dir):
    process_sample(sample_id, bam_path, ref, bed_dir, allele_ref_dir)
...
if __name__ == "__main__":
    ...
    main(args.sample_id, args.bam_path, args.ref, args.bed_dir, args.allele_ref_dir)
