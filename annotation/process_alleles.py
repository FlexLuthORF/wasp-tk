...
def process_locus(sample_id, bam_path, locus, loci, ref, bed_dir, allele_ref_dir, outdir):
    ...
    make_command = f"python /usr/local/bin/make_gene_file.py --sample {sample_id} {locus} {loci} \"+-\" {bed_dir} {ref} {make_gene_file_output} -b {bam_path}"
    ...
    import_command = f"python /usr/local/bin/import_from_assemblies.py {locus} {loci} \"+-\" {make_gene_file_output} {ref} {bed_dir} {allele_ref_dir} {import_from_assembly_output}"
